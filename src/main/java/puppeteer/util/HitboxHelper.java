...
public class HitboxHelper {
    ...
    public static boolean testLineIntersect(float p0_x, float p0_y, float p1_x, float p1_y,
    ...
    public static AbstractDoll getClosestDoll(float x, float y)
    {
        AbstractDoll closest = null;
        ArrayList<AbstractDoll> dolls = new ArrayList<>(DollFields.dolls.get(AbstractDungeon.player));
        if (dolls.isEmpty())
        {
            return closest;
        }
        ...
        for (AbstractDoll d : dolls)
        {
            ...
            {
                closest = d;
                minDist = dist(x, d.x, y, d.y);
            }
            else if (Math.abs(d.x - x) < minDist && Math.abs(d.y - y) < minDist)
            {
                float dist = dist(x, d.x, y, d.y);
                if (dist < minDist)
                {
                    closest = d;
                    minDist = dist;
